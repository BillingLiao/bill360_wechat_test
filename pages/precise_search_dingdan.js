...
var precise_search_dingdan = function (_wepy$page) {
  _inherits(precise_search_dingdan, _wepy$page);

  function precise_search_dingdan() {
    var _ref;

    var _temp, _this, _ret;

    _classCallCheck(this, precise_search_dingdan);

    for (var _len = arguments.length, args = Array(_len), _key = 0; _key < _len; _key++) {
      args[_key] = arguments[_key];
    }

    return _ret = (_temp = (_this = _possibleConstructorReturn(this, (_ref = precise_search_dingdan.__proto__ || Object.getPrototypeOf(precise_search_dingdan)).call.apply(_ref, [this].concat(args))), _this), _this.config = {
      ...
      getData: function getData() {
        ...
        wx.request({
          url: 'https://xcx.wenxikeji.com//bill360/order/add',
          method: 'POST',
          data: {
            token: that.token,
            subsidy: that.ordAddBillSubsidy,
            interest: that.ordAddBillInterest,
            day: that.ordAddBillDay,
            billId: that.ordAddId,
            money: that.ordAddBillMoney,
            time: that.ordAddBillTime,
            img: that.ordAddBillImg
          },
  ...
  _createClass(precise_search_dingdan, [{
    key: 'onLoad',
    value: function onLoad() {
      ...
      wx.getStorage({
        key: 'preciseImg',
        success: function success(res) {
          that.ordAddBillImg = 'https://xcx.wenxikeji.com/bill360/upload/' + res.data;
        }
      });
      wx.getStorage({
        key: 'rate',
      ...
      wx.getStorage({
        ...
        success: function success(res) {
          ...
          that.ordAddBillCore = res.data.acceptance.core;
          that.ordAddBillInvoice = res.data.acceptance.invoice;
           that.ordAddBillName = res.data.staff.name;
           that.ordAddBillPhone = res.data.staff.phone;
  ...
  return precise_search_dingdan;
}(_wepy2.default.page);


Page(require('./../npm/wepy/lib/wepy.js').default.$createPage(precise_search_dingdan , 'pages/precise_search_dingdan'));
